  ...
  completers = {
    omni: new MultiCompleter(
      [
        ...
        Settings.get('searchHistoryStatus') ? completionSources.history : null,
        Settings.get('searchBookmarkStatus') ? completionSources.bookmarks : null,
        Settings.get('searchTabsStatus') ? completionSources.tabs : null,
      ].filter(v => v)
    ),
      ...
      chrome.webNavigation.getAllFrames({ tabId }, function(frames) {
        ...
        for (let index = 0; index < frames.length; index++) {
          ...
          for (const key in iframeMap) {
            if(!iframeMap[key] || !iframeMap[key].url) {
              continue
            }
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if(!request.source || (request.source && request.source !== 'GM_Content' && request.source !== 'GM_Popup')) {
    if (Settings.debug) {
