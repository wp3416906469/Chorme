            ...
            port1.onmessage = event => {
              ...
              switch (eventName) {
                case 'uiComponentIsReady':
                  ...
                  /*
                    问题描述：如果是help 点击链接，搜索框会消失，无法显示内置页面
                    修改理由：允许用户在搜索框下操作网页
                    修改事件为click,实际测试只有点击搜索框外部页面才会触发click事件
                   */
                  window.addEventListener(
                    'click',
                    forTrusted(event => {
                      if (event.view === window && this.options && this.options.focus) {
                        this.hide(false);
                      }
      ...
      setTimeout(() => {
        try {
          if (location.href.startsWith(chrome.runtime.getURL('/'))) {
            window.CSDNVomnibar = this.showing;
          }
        } catch (e) {}
      }, 300);
