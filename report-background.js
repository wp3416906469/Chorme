/* eslint-disable */
(function($) {
  var CFG, exports, _fn, REPORT_TYPE, requestDataArr, defaultData, VIEW_STRATEGY, SERVER_URL, utmData, VERSION, UserInfo, CUSTOM_HREF, CUSTOM_HOST, EXTENSION_ID, pluginSource;
  requestDataArr = [];
  CFG = {
    DELAY: 500,
    API_VERSION: '0.6.0',
  ...
  CUSTOM_HREF = window.customReportHref || 'https://plugin.csdn.net/packages/search-box/index.html';
  ...
  _fn = {
    sendAjaxRequest: function(request) {
      chrome.storage.local.get('installUUID', function(items) {
        ...
        if (request.type && request.type.toLowerCase() === 'post') {
    ...
    getUserInfo: function() {
      return new Promise(resolve => {
        ...
        chrome.cookies.getAll({ url: 'https://csdn.net' }, function(res) {
          ...
          chrome.cookies.get({ name: 'pluginSource', url: 'https://plugin.csdn.net' }, function(cookieRes) {
            if (cookieRes) {
              pluginSource = cookieRes.value;
            }
            resolve(UserInfo);
          });
        });
    ...
    initUTM: async function() {
      ...
      if (JSON.stringify(request) !== '{}') {
        ...
        // 清空所有utm_参数
        if (clearAll) {
          var cCookie = await _fn.getFuzzyCookie('c_utm_');
    ...
    getFuzzyCookie: function(fuzzyQuery) {
      return new Promise(res => {
        chrome.cookies.getAll({ url: 'https://csdn.net' }, function(e) {
          res(
            e
              .filter(i => new RegExp(`^${fuzzyQuery}`).exec(i.name))
  ...
  exports = {
    ...
    reportView: async function(data, thisObj, delay) {
      if (!data) {
        console.warn('reportView Error:', data);
        return;
      }
      var newData = $.extend(true, {}, defaultData, data);
      var cCookie = await _fn.getFuzzyCookie('c_');
      ...
      newData['curl'] = data ? data.curl || CUSTOM_HREF : CUSTOM_HREF;
      ...
      var reportFunc = function(_data, _newData, _delay, _thisObj) {
        ...
        _newData.uid = UserInfo.UserName;
     ...
     * CLICK上报
     */
    reportClick: async function(data, thisObj) {
      var newData = $.extend(true, {}, defaultData, data);
      if (!data.spm) {
        newData.spm = '';
      }
     ...
     * @param data  自定义参数
     */
    reportPageView: async function(data) {
      var newData = $.extend(true, {}, defaultData, data);
      newData.tos = _fn.tos();
  ...
  _fn.initData().then(_defaultData => {
