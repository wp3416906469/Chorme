...
const presuppositionName = BgUtils.getPresuppositionName();
...
class BookmarkCompleter {
  filter({ queryTerms, query }, onComplete) {
    ...
    const useBmCommand = BgUtils.usePresupposition(queryTerms, 'bm');

    if (useBmCommand) {
      queryTerms.splice(0, 1);
    }
    if (queryTerms.length && excludeKeys.indexOf(query.split(/\s+/)[0]) > -1 && !useBmCommand) {
      return onComplete([]);
    }
    this.query = query;
  ...
  performSearch() {
    ...
    } else if (this.query === 'bm ' || this.query === 'bm' || this.query === 'bookmark' || this.query === 'bookmark ') {
  ...
  versionCompleter(queryTerms, onComplete) {
    onComplete([
      new Suggestion({
        ...
        url: 'javascript:;',
        description: ``,
        title: presuppositionName['version'],
    ...
    onComplete([
      new Suggestion({
        ...
        info: {
          ...
          height: '460px',
          extendType: 'qrcode',
          params: queryTerms.length > 1 ? queryTerms.slice(1).join(' ') : '',
          url: chrome.runtime.getURL('/') + 'pages/qrcode.html',
        },
        autoSelect: true,
        description: `二维码工具`,
        title: '二维码工具',
    ...
    if (!Array.isArray(queryTerms) || queryTerms.length === 0) {
      ...
      try {
        if (iframeMap.hasOwnProperty(firstQuery) && iframeMap[firstQuery].enable) {
            ...
            if (queryTerms.length === 1 && query.indexOf(' ') > -1) {
              ...
              result.push(
                new Suggestion({
                  ...
                  info: info,
                  autoSelect: true,
                  description: `iframe工具-${info.name || ''}`,
                  title: info.name || `iframe工具`,
                  highlightTerms: false,
                  highlightTermsExcludeUrl: false,
                  isCustomSearch: true,
                  relevancy: 3,
          ...
          let webpageUrl = '';
          if (webIframe.pluginId) {
            const dynamicPluginURL = chrome.runtime.getURL('/pages/dynamicPlugin.html');
            webpageUrl = `${dynamicPluginURL}?pluginId=${webIframe.pluginId}&query=${encodeURIComponent(query)}`;
          } else if (webIframe.url) {
            webpageUrl = webIframe.url.indexOf('?') > -1 ? `${webIframe.url}&query=${encodeURIComponent(query)}` : `${webIframe.url}?query=${encodeURIComponent(query)}`;
          } else {
            onComplete([]);
            return;
          }
        ...
        continuation: onComplete => {
          $.get('https://so.csdn.net/api/v1/search?platform=pc&s=5&from=chrome_plugin&q=' + encodeURIComponent(queryTerms.join(' ')), res => {
            if (res.result_vos && res.result_vos.length) {
              ...
              this.csdn = results.splice(0, maxResults);
              try {
                onComplete(
...
class HistoryCompleter {
  filter({ queryTerms, seenTabToOpenCompletionList, query }, onComplete) {
    const excludeKeys = BgUtils.getExcludeKeys();
    const useHisCommand = BgUtils.usePresupposition(queryTerms, 'his');

    if ((queryTerms.length === 0 && !seenTabToOpenCompletionList) || (queryTerms.length && excludeKeys.indexOf(query.split(/\s+/)[0]) > -1 && !useHisCommand)) {
      onComplete([]);
